const MainCategory = ({ categoriesData }) => {
  const getStrapiMediaUrl = (url) => {
    return `${process.env.REACT_APP_BACKEND_URL || 'http://localhost:1337'}${url}`;
  };
    ...
    <div>
      <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 lg:gap-8">
        {categoriesData.data.map((category) => {
          const imageUrl = getStrapiMediaUrl(category.attributes.img.data.attributes.formats.medium.url);
          return (
